...
3. Run the full automated pipeline (ingest → preprocess → train → test → build & run Docker):
...
> 🔧 Note: Input feature order must match training: `Time`, `V1`–`V28`, `Amount` (total 30 features).
...
├── src/              # Python scripts for ingestion, preprocessing, training
├── tests/            # Pytest unit tests for data and model validation
├── frontend/         # Streamlit dashboard code
├── app.py            # FastAPI backend logic
├── Dockerfile        # Container configuration
└── run_pipeline.ps1  # Full automation script
```
